...
#' @param burnin the first number of posterior sample is discard. 
...
BayesRobustProbitSummary = function(object, digits = max(1L, getOption("digits") - 4L), burnin)
{
	...
	if(object$call[1] == "BayesProbitHSD()")
		output = BayesProbitHSD.Summary(object, digits = digits, BurnIn = burnin)
